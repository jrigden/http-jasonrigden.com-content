...
![](https://cdn-images-1.medium.com/max/800/1*wgj99sjyGKRtrO9XaXtOoA.jpeg)
...
r = requests.get**(**'[https://jasonrigden.com'](http://httpbin.org/ip%27)**)**

But the using the Session object is a just a tiny bit different:

r = session.get('[https://jasonrigden.com](http://httpbin.org/ip%27)['](https://icanhazip.com%27/))

### Checking our current IP

_I’ll will not be showing my my real IP address. Instead. I’ll be using your routers address 192.168.0.1._ 😉

Let us check out IP address by asking  [httpbin.org](https://httpbin.org/).

r = session.get(‘[http://httpbin.org/ip'](http://httpbin.org/ip%27))  
print(r.text)
...
r = session.get('[https://www.facebookcorewwwi.onion/](https://www.facebookcorewwwi.onion/)['](http://httpbin.org/ip%27))  
print(r.headers)
...
r = session.get(‘[https://httpbin.org/user-agent'](https://httpbin.org/user-agent%27))  
print(r.text)
...
r = session.get('[https://httpbin.org/user-agent'](https://httpbin.org/user-agent%27), headers=headers)  
print(r.text)
...
session.get('[http://httpbin.org/cookies/set/sessioncookie/Hello'](http://httpbin.org/cookies/set/sessioncookie/Hello_Dolly%27))

Now we check to see if that cookie is working.:

r = session.get(‘[http://httpbin.org/cookies'](http://httpbin.org/cookies%27))  
print(r.text)
...
requests.get(‘[http://httpbin.org/ip'](http://httpbin.org/ip%27))

![](https://cdn-images-1.medium.com/max/800/1*xC_nF3fSmJ1JLdVUmkUJHw.png)

But using the proxies through the Session object reveals nothing.

![](https://cdn-images-1.medium.com/max/800/1*zGFfttJAKkjURouIkMchNg.png)
